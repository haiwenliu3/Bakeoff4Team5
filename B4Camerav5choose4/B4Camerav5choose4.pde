...
String[] colors = {"Red", "Green", "Blue", "Yellow"}; // COLOR NAMES CORRESPOND TO RGB VALUES ON NEXT LINE (whites, grays, and yellow seem to be BAD)
int[][] rgbs = { {210,0,0}, {0,137,13}, {0,96,255}, {120,120,0} }; // put values here based on rgb camera reading in DEBUGGING MODE
...
void draw() {
  ...
  if (cam.isStarted() && chooseStage == 4 && !debug)
  {
    ...
    text("Target: "+colors[targets.get(index).target], width*.03 , height*.1);
  ...
  {
    String[] options2={"Left", "Right"};
    textFont(font, 100);
    fill(0);
    stroke(0);
    text("Target: "+options2[targets.get(index).action], width*.03 , height*.1);
    text("Trial: "+(index+1)+"/"+trialCount, width*.03 , height*.2);
...
void drawColor4UI()
{
  float tileSize = height*0.45;
  float centerX = width*.7;
  float centerY = height*.5;
  int targetOption = targets.get(trialIndex).target;
  //First tile
...
void drawDebug()
{
  ...
  textFont(font, 100);
  text("Target: "+colors[targets.get(trialIndex).target], width*.03 , height*.1);
...
{
  if (confirmedColor == colors[targets.get(trialIndex).target])
  {
...
void checkNext2(boolean secondStageCorrect) // MODIFY THIS TO CHECK IF BOTH STAGES WERE CORRECT AFTER CHOOSE 2
{
  if (secondStageCorrect && firstStageCorrect)
  {
    trialIndex++;
  }
  chooseStage = 4;
